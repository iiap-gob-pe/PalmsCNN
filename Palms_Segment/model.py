...
from tensorflow.keras.layers import (
    Input, Activation, Concatenate, Add, Dropout, BatchNormalization, 
    Conv2D, DepthwiseConv2D, ZeroPadding2D, AveragePooling2D, Layer, InputSpec
)
    ...
    """def call(self, inputs):
        if self.upsampling:
            ...
            return K.tf.image.resize_bilinear(inputs, (self.output_size[0],
                                              ...
                                              align_corners=True)"""
    def call(self, inputs):
        input_shape = tf.shape(inputs)  # Dynamic shape at runtime
        if self.upsampling:
            # Use dynamic shape instead of static inputs.shape[1] and inputs.shape[2]
            height = input_shape[1] * self.upsampling[0]
            width = input_shape[2] * self.upsampling[1]
            return tf.compat.v1.image.resize_bilinear(inputs, (height, width), align_corners=True)
    
        else:
            # If output size is explicitly given
            return tf.compat.v1.image.resize_bilinear(inputs, (self.output_size[0], self.output_size[1]), align_corners=True)
...
def dice_coef(y_true, y_pred, smooth=1): #F1 score = mean (2*(|X & Y|)/ (|X + Y|))
