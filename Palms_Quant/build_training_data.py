...
def get_feature_scaling(filelist, scale_flag, nodata_value=-9999):
    ...
    Keyword Arguments:
    nodata_value - value to be ignored, None of no nodata_value specified
...
def get_feature_scaling_from_array(filelist, scale_flag, nodata_value=-9999):
    """ Loop through the file list and determine feature scaling based on the entire set.
    Arguments:
    filelist - list of file names to scan through  
    scale_flag - determine which type of scaling to use
    ...
    if(len(filelist.shape)>2):
      RasterCount=filelist.shape[2]
    else:
      RasterCount=1
    scale_factors = np.ones((filelist.shape[2], 2)) #new array 
    scale_factors[:, 0] = 0
    if scale_flag is None:
        return scale_factors

    full_dat = None
    for b in range(0, RasterCount):
        
        local_dat = filelist
        if (len(local_dat.shape) == 2): #check number of elements
          local_dat = np.reshape(local_dat,(local_dat.shape[0],local_dat.shape[1],1)) 
        ...
        local_scale_factor = scale_vector(full_dat, scale_flag, nodata_value=nodata_value) #normalization
        scale_factors[b, 0] = local_scale_factor[0]
        scale_factors[b, 1] = local_scale_factor[1]
        del full_dat
...
def build_semantic_segmentation_training_data(window_radius, samples_per_response_per_site, feature_file_list, response_file_list, response_vector_flag=True, boundary_file_list = [], boundary_file_vector_flag = True, boundary_bad_value=0, internal_window_radius=None, center_random_offset_fraction=0.0, response_repeats=1,savename=None, nodata_maximum_fraction=0.5, response_minimum_fraction=0.0, fill_in_feature_data=True, global_scale_flag=None, local_scale_flag=None, nodata_value=-9999, random_seed=13, n_folds=10, verbose=False, ignore_projections=False):
  ...
  """ Main externally called function, transforms a list of feature/response rasters into 
	...
	boundary_file_list - optional list of boundary files for each feature/response file. Helps to restrain the mosaic extent if the response file is smaller
  ...
  for _i in range(0,len(feature_file_list)):
    ...
    if(verbose): print("un_response ", un_response.shape)
    if (isinstance(samples_per_response_per_site,list)):
...
def build_semantic_segmentation_training_data_from_response_array(window_radius, samples_per_response_per_site, feature_file_list, response_file_list, gt_responses_array,response_vector_flag=True, boundary_file_list = [], boundary_file_vector_flag = True, boundary_bad_value=0, internal_window_radius=None, center_random_offset_fraction=0.0, response_repeats=1,savename=None, nodata_maximum_fraction=0.5, response_minimum_fraction=0.0, fill_in_feature_data=True, global_scale_flag=None, local_scale_flag=None, nodata_value=-9999, random_seed=13, n_folds=10, verbose=False, ignore_projections=False):
  ...
  #Check the tiff file list
  #if(isinstance(response_file_list,list)):
  #  check_data_matches(feature_file_list,response_file_list,response_vector_flag,boundary_file_list,boundary_file_vector_flag,ignore_projections)
  ...
  n_features = np.nan
  if(verbose): print("feature_scaling")
  for _i in range(0,len(feature_file_list)):
    
    # open requisite datasets
    if(verbose): print("load feature_file_list")
    dataset = gdal.Open(feature_file_list[_i],gdal.GA_ReadOnly)
    if (np.isnan(n_features)):
      n_features = dataset.RasterCount
    feature = np.zeros((dataset.RasterYSize,dataset.RasterXSize,dataset.RasterCount))
    ...
    if(verbose): print("load response_file_list")
    if(isinstance(response_file_list,list)):
      if (response_vector_flag):
        response = rasterize_vector(response_file_list[_i],dataset.GetGeoTransform(),[feature.shape[0],feature.shape[1]])
      else:
      ...
      response=response_file_list

    if(verbose): print("response.shape",response.shape)
    if(verbose): print("gt_responses.shape",gt_responses_array.shape)
    if (len(boundary_file_list) > 0):
      if (boundary_file_list[_i] is not None):
        if (response_vector_flag):
          mask = rasterize_vector(boundary_file_list[_i],dataset.GetGeoTransform(),[feature.shape[0],feature.shape[1]])
        ...
        feature[mask == boundary_bad_value,:] = nodata_value
        response[mask == boundary_bad_value] = nodata_value
        responsegt = gdal.Open(responsegt_file_list[_i]).ReadAsArray().astype(float)


    if(verbose): print(feature.shape)
    # ensure nodata values are consistent 
    if(verbose): print("ensure nodata values are consistent ")
    if (not dataset.GetRasterBand(1).GetNoDataValue() is None):
      feature[feature == dataset.GetRasterBand(1).GetNoDataValue()] = nodata_value
    feature[np.isnan(feature)] = nodata_value
    feature[np.isinf(feature)] = nodata_value
    ...
    if(len(response.shape) >2):
      feature[response[:,:,0] == nodata_value,:] = nodata_value
    else:
      feature[response == nodata_value,:] = nodata_value

    if(verbose): print("Escalando  feature")
    for n in range(0,feature.shape[2]):
      gd = feature[:,:,n] != nodata_value
      feature[gd,n] = feature[gd,n] - feature_scaling[n,0]
      feature[gd,n] = feature[gd,n] / feature_scaling[n,1]
    ...
    # loop through each unique response
    if(verbose): print("loop through each unique response") 
    for ur in un_response:
      coords = np.where(response == ur)
      if(verbose): print((len(coords[0]),'response locations potentially available'))
      perm = np.random.permutation(len(coords[0]))
      ...
      for repeat in range(0,response_repeats):
        ...
        while (pos_len < lsps and n < len(coords[0])):
          ...
          if ((np.sum(d == nodata_value) <= d.size * nodata_maximum_fraction)):
           if(d.shape[0] == window_radius*2 and d.shape[1] == window_radius*2):
            r = response[coords[0][n]-window_radius:coords[0][n]+window_radius,coords[1][n]-window_radius:coords[1][n]+window_radius].copy()
            gt_r = gt_responses_array[coords[0][n]-window_radius:coords[0][n]+window_radius,coords[1][n]-window_radius:coords[1][n]+window_radius].copy()
            if (np.sum(r == ur) > r.size * response_minimum_fraction):
              responses.append(r)
              gt_responses.append(gt_r)
              d = scale_image(d,local_scale_flag,nodata_value=nodata_value)
              if (fill_in_feature_data == True):
               if (np.sum(d == nodata_value) > 0):
                d = fill_nearest_neighbor(d)
  ...
  features = np.stack(features)
  responses = np.stack(responses)
  gt_responses = np.stack(gt_responses)
  repeat_index = np.stack(repeat_index)
  ...
  features = features[perm,:]
  responses = responses[perm,:]
  gt_responses = gt_responses[perm,:]
  repeat_index = repeat_index[perm]
  fold_assignments = np.zeros(responses.shape[0])
  ...
  features = features.reshape((features.shape[0],features.shape[1],features.shape[2],n_features))
  responses = responses.reshape((responses.shape[0],responses.shape[1],responses.shape[2],1))
  gt_responses = gt_responses.reshape((gt_responses.shape[0],gt_responses.shape[1],gt_responses.shape[2],2))
  ...
  if (savename is not None):
    np.savez(savename,features=features,responses=responses,gt_responses=gt_responses,fold_assignments=fold_assignments)

  return features,responses,fold_assignments,gt_responses


def build_semantic_segmentation_training_data_for_DWT(window_radius, samples_per_response_per_site, feature_file_list, response_file_list,responsegt_file_list, response_vector_flag=True,responsegt_vector_flag=True, boundary_file_list = [], boundary_file_vector_flag = True, boundary_bad_value=0, internal_window_radius=None, center_random_offset_fraction=0.0, response_repeats=1,savename=None, nodata_maximum_fraction=0.5, response_minimum_fraction=0.0, fill_in_feature_data=True, global_scale_flag=None, local_scale_flag=None, nodata_value=-9999, random_seed=13, n_folds=10, verbose=False, ignore_projections=False):
  ...
  """ Main externally called function, transforms a list of feature/response rasters into 
    ...
    feature_file_list - file list of the feature rasters
    response_file_list - file list of the response rasters
    responsegt_file_list - file list of the Ground truth rasters
  ...
  check_data_matches(feature_file_list,response_file_list,response_vector_flag,boundary_file_list,boundary_file_vector_flag,ignore_projections)
  check_data_matches(feature_file_list,responsegt_file_list,responsegt_vector_flag,boundary_file_list,boundary_file_vector_flag,ignore_projections)
  ...
  for _i in range(0,len(feature_file_list)):
    ...
    # open requisite datasets
    print("feature_file_list",feature_file_list[_i])
    dataset = gdal.Open(feature_file_list[_i],gdal.GA_ReadOnly)
    if (np.isnan(n_features)):
      n_features = dataset.RasterCount
    feature = np.zeros((dataset.RasterYSize,dataset.RasterXSize,dataset.RasterCount))
    ...
    if (responsegt_vector_flag):
      responsegt = rasterize_vector(responsegt_file_list[_i],dataset.GetGeoTransform(),[feature.shape[0],feature.shape[1]])
    else:
      responsegt = gdal.Open(responsegt_file_list[_i]).ReadAsArray().astype(float)
    ...
    if (len(boundary_file_list) > 0):
      if (boundary_file_list[_i] is not None):
        ...
        feature[mask == boundary_bad_value,:] = nodata_value
        response[mask == boundary_bad_value] = nodata_value
        responsegt[mask == boundary_bad_value] = nodata_value



    if(verbose): 
      print("feature",feature.shape)
      print("responsegt",responsegt.shape)
    # ensure nodata values are consistent 
    if (not dataset.GetRasterBand(1).GetNoDataValue() is None):
      feature[feature == dataset.GetRasterBand(1).GetNoDataValue()] = nodata_value
    feature[np.isnan(feature)] = nodata_value
    ...
    response[feature[:,:,0] == nodata_value] = nodata_value
    feature[response == nodata_value,:] = nodata_value
    responsegt[feature[:,:,0] == nodata_value] = nodata_value

    #Generate Depth and direction maps
    from scipy import ndimage
    depth_map=ndimage.distance_transform_edt(responsegt)
    dir_map=np.zeros((depth_map.shape[0],depth_map.shape[1],2),dtype=np.float32)
    dir_map[:,:,0] = ndimage.sobel(depth_map,1)
    dir_map[:,:,1]  = ndimage.sobel(depth_map,0)
      ...
      for repeat in range(0,response_repeats):
        ...
        while (pos_len < lsps and n < len(coords[0])):
          ...
          if ((np.sum(d == nodata_value) <= d.size * nodata_maximum_fraction)):
           if(d.shape[0] == window_radius*2 and d.shape[1] == window_radius*2):
            r = response[coords[0][n]-window_radius:coords[0][n]+window_radius,coords[1][n]-window_radius:coords[1][n]+window_radius].copy()
            deepm = depth_map[coords[0][n]-window_radius:coords[0][n]+window_radius,coords[1][n]-window_radius:coords[1][n]+window_radius].copy()
            dirm = dir_map[coords[0][n]-window_radius:coords[0][n]+window_radius,coords[1][n]-window_radius:coords[1][n]+window_radius].copy()
            if (np.sum(r == ur) > r.size * response_minimum_fraction):
              responses.append(r)
              depth_maps.append(deepm)
              dir_maps.append(dirm)
              #d = scale_image(d,local_scale_flag,nodata_value=nodata_value)
              #if (fill_in_feature_data == True):
              # if (np.sum(d == nodata_value) > 0):
              #  d = fill_nearest_neighbor(d)
  ...
  features = features.reshape((features.shape[0],features.shape[1],features.shape[2],n_features))
  responses = responses.reshape((responses.shape[0],responses.shape[1],responses.shape[2],1))
  depth_maps = depth_maps.reshape((depth_maps.shape[0],depth_maps.shape[1],depth_maps.shape[2],1))
  dir_maps = dir_maps.reshape((dir_maps.shape[0],dir_maps.shape[1],dir_maps.shape[2],2))


  print(('feature shape',features.shape))
  print(('name_responses shape',name_responses.shape))
  print(('depth_maps shape',depth_maps.shape))
  print(('dir_maps shape',dir_maps.shape))
  print("========= Generating ",features.shape[0]," Tiles=========")
  
  return features,responses,fold_assignments,depth_maps,dir_maps,name_responses

def build_semantic_segmentation_training_data_for_DWT_removedx(window_radius, samples_per_response_per_site, feature_file_list, response_file_list,responsegt_file_list, response_vector_flag=True,responsegt_vector_flag=True, boundary_file_list = [], boundary_file_vector_flag = True, boundary_bad_value=0, internal_window_radius=None, center_random_offset_fraction=0.0, response_repeats=1,savename=None, nodata_maximum_fraction=0.5, response_minimum_fraction=0.0, fill_in_feature_data=True, global_scale_flag=None, local_scale_flag=None, nodata_value=-9999, random_seed=13, n_folds=10, verbose=False, ignore_projections=False,remove_distance=0):
  ...
  for _i in range(0,len(feature_file_list)):
    ...
    from scipy import ndimage
    depth_map=ndimage.distance_transform_edt(responsegt)
    depth_map[depth_map<=remove_distance]=0
    depth_map[depth_map>0]=1
    depth_map=ndimage.distance_transform_edt(depth_map)
    dir_map=np.zeros((depth_map.shape[0],depth_map.shape[1],2),dtype=np.float32)
    dir_map[:,:,0] = ndimage.sobel(depth_map,1)
    dir_map[:,:,1]  = ndimage.sobel(depth_map,0)
