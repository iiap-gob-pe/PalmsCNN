...
def ssProcess_lists(ssImages):
    ssMasks=[]
    ssBinarys=[]
    for i in range(len(ssImages)):
        ssBinary, ssMask=ssProcess(ssImages[i])
        ssBinarys.append(ssBinary)
        ssMasks.append(ssMask)
    ssBinarys=np.array(ssBinarys)
    ssMasks=np.array(ssMasks)
    return ssBinarys,ssMasks
